...
import { generateKundli, babyNameSuggestions } from "@services/api/kundli";
...
export default function BabyNamesScreen() {
  ...
  useEffect(() => {
    (async () => { try { const svc = await getServiceBySlug("baby-name"); const paid = !!svc?.isPaid; setIsPaid(paid); if (paid) { setServicePrice(svc.basePrice || 0); setActivated(false); } else { setActivated(true); } } catch {} })();
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await babyNameSuggestions({ chart: kundli.chart, preferences: prefs ? { note: prefs } : undefined });
      setResult(res);
    } catch (e: any) { Alert.alert("Failed", e?.message || "Could not generate suggestions"); }
    finally { setLoading(false); }
  };

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.background }}>
      <Header title="Baby Name Suggestion" />
      <KeyboardAvoidingView behavior={Platform.select({ ios: "padding", android: undefined })} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
          <Card>
            {isPaid && servicePrice !== null && (
              <View style={{ marginBottom: 12 }}>
                ...
                <TouchableOpacity onPress={async () => {
                  setPaying(true);
                  try { await initiatePayment({ serviceType: "baby_name", items: [], gateway: "wallet" }); setActivated(true); Alert.alert("Activated", "Baby names activated via wallet"); }
            ...
            <Text style={{ fontWeight: "700", marginBottom: 8 }}>Birth Details</Text>
            ...
            <Text style={{ fontWeight: "700", marginVertical: 8 }}>Preferences</Text>
            <TextInput placeholder="Optional preferences (e.g., starting letter)" placeholderTextColor={c.textSoft} value={prefs} onChangeText={setPrefs} style={{ borderWidth: 1, borderColor: c.border, borderRadius: 12, padding: 12, marginBottom: 8, color: c.text }} />
            <TouchableOpacity onPress={submit} disabled={loading || (isPaid && !activated)} style={{ backgroundColor: c.primary, padding: 12, borderRadius: 12, opacity: (isPaid && !activated) ? 0.6 : 1 }}>
              <Text style={{ color: c.primaryForeground, textAlign: "center", fontWeight: "700" }}>{(isPaid && !activated) ? "Activate to Continue" : (loading ? "Generating..." : "Get Suggestions")}</Text>
            </TouchableOpacity>
          </Card>

          {result && (
            <Card style={{ marginTop: 12 }}>
              <Text style={{ fontSize: 16, fontWeight: "800", marginBottom: 6 }}>Suggestions</Text>
              {Array.isArray(result.suggestions) ? (
                <Markdown style={{ body: { color: c.text } }}>{"\n" + result.suggestions.map((s: string) => `- ${s}`).join("\n")}</Markdown>
              ) : (
                <Markdown style={{ body: { color: c.text } }}>{result.narrative || ""}</Markdown>
              )}
            </Card>
          )}
          {(loading || paying) && <FullScreenLoader label={loading ? "Generating..." : "Processing..."} />}
        </ScrollView>
