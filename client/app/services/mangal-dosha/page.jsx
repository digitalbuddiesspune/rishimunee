...
function MangalDoshaContent() {
  ...
  const onSubmit = async (values) => {
    try {
      setErrorMessage("");
      // Charge only if user hasn't come from checkout
      if (!alreadyPaid) {
        await apiClient.post("/payments/initiate", { serviceType: "mangal_dosha", gateway: PAYMENT_GATEWAYS.WALLET });
      }
      const gen = await apiClient.post("/kundli/generate", values);
      const chart = gen?.data?.data?.chart;
      const { data } = await apiClient.post("/kundli/mangal-dosha", { chart });
      setResult({ chart, narrative: data?.data?.narrative, analysis: data?.data?.analysis });
      dispatch(fetchWallet());
    } catch (error) {
      const message = error.response?.data?.message || "Failed to evaluate Mangal Dosha";
  ...
  // Result mode: show left nav + analysis
  if (result) {
    return (
      <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-12 md:grid-cols-12">
        <aside className="md:col-span-4 lg:col-span-3">
          <ServiceNav />
          <ServiceHistory serviceType="mangal_dosha" onSelect={(r) => { setFromHistory(true); setResult({ chart: r.result?.chart || null, narrative: r.result?.narrative, analysis: r.result?.analysis }); }} />
        </aside>
    ...
    <div className="mx-auto grid w-full max-w-5xl gap-8 px-4 py-12 md:grid-cols-[1.1fr,0.9fr]">
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? "Analysing..." : "Check Mangal Dosha"}
            </Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-[color:var(--color-text-soft)]">
          <p>Enter birth details to generate the chart and evaluate Mangal Dosha impact and remedies.</p>
          {alreadyPaid && <p className="text-xs">Payment confirmed. You can proceed to generate.</p>}
        </CardContent>
      </Card>
      <LoadingOverlay show={isSubmitting} label="Checking Mangal Dosha..." />
    </div>
  );
}
