...
function CareerCounsellingContent() {
  ...
  const { register, handleSubmit, formState: { isSubmitting }, reset } = useForm({
    defaultValues: { name: "", date: "", time: "", place: "", goals: "" }
  });
  ...
  const onSubmit = async (values) => {
    try {
      setErrorMessage("");
      if (!alreadyPaid) {
        await apiClient.post("/payments/initiate", { serviceType: "career_counselling", gateway: PAYMENT_GATEWAYS.WALLET });
      }
      const gen = await apiClient.post("/kundli/generate", values);
      const chart = gen?.data?.data?.chart;
      const { data } = await apiClient.post("/kundli/career", { chart, goals: values.goals });
      setResult({ chart, narrative: data?.data?.narrative });
      dispatch(fetchWallet());
    } catch (error) {
      const message = error.response?.data?.message || "Failed to generate career guidance";
    ...
    return (
      <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-12 md:grid-cols-12">
        <aside className="md:col-span-4 lg:col-span-3">
          <ServiceNav />
          <ServiceHistory serviceType="career_counselling" onSelect={(r) => { setFromHistory(true); setResult({ chart: r.result?.chart || null, narrative: r.result?.narrative }); }} />
        </aside>
        <main className="md:col-span-8 lg:col-span-9">
          <Card className="h-[70vh] flex flex-col">
            <CardHeader><CardTitle>Career Counselling</CardTitle></CardHeader>
    ...
    <div className="mx-auto grid w-full max-w-5xl gap-8 px-4 py-12 md:grid-cols-[1.1fr,0.9fr]">
      <Card>
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <TextArea rows={4} placeholder="Your current situation and goals (job change, skills, location, salary, timeline)" {...register("goals", { required: true })} />
            <div className="flex items-center gap-2">
              <Button type="submit" disabled={isSubmitting}>{isSubmitting ? "Analysing..." : "Get Guidance"}</Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-[color:var(--color-text-soft)]">
          <p>Personalised career guidance using your birth chart. Includes strengths, timing, and recommended actions.</p>
          {alreadyPaid && <p className="text-xs">Payment confirmed. You can proceed to generate.</p>}
        </CardContent>
      </Card>
      <LoadingOverlay show={isSubmitting} label="Analysing for career guidance..." />
    </div>
  );
}
