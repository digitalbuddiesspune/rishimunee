...
export default function GocharPhalScreen() {
  ...
  const load = async (sign: string) => {
    ...
    try {
      ...
      setError(e?.message || "Failed to load Gochar Phal");
    } finally { setLoading(false); }
  };

  useEffect(() => { load(active); }, [active]);

  // If server extends to include transit field later, prefer it otherwise show daily summary as today's transit impact
  const transit = (record as any)?.transit || record?.daily?.summary || "";

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.background }}>
      <Header title="Gochar Phal" />
        ...
        <Card>
          {loading ? (
            ...
            <>
              ...
              <Markdown style={{ body: { color: c.text } }}>{transit || ""}</Markdown>
            </>
          )}
        </Card>
      </ScrollView>
