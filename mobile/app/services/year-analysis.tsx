...
import { generateYearAnalysis } from "@services/api/serviceReports";
...
export default function YearAnalysisScreen() {
  ...
  useEffect(() => {
    (async () => { try { const svc = await getServiceBySlug("year-analysis"); const paid = !!svc?.isPaid; setIsPaid(paid); if (paid) { setServicePrice(svc.basePrice || 0); setActivated(false); } else { setActivated(true); } } catch {} })();
  }, []);
  ...
  const submit = async () => {
    ...
    setResult(null);
    try {
      const data = await generateYearAnalysis({ notes, ...form });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.background }}>
      <Header title="Year Analysis (वार्षिक विश्लेषण)" />
      <KeyboardAvoidingView behavior={Platform.select({ ios: "padding", android: undefined })} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
          <Card>
            {isPaid && servicePrice !== null && !activated && (
              <View style={{ marginBottom: 12 }}>
                ...
                <TouchableOpacity onPress={async () => {
                  setPaying(true);
                  try { await initiatePayment({ serviceType: "year_analysis", items: [], gateway: "wallet" }); setActivated(true); Alert.alert("Activated", "Year analysis activated via wallet"); }
            ...
            <TextInput placeholder="Add any focus areas for this year's analysis (optional) • इस वर्ष के विश्लेषण के फोकस क्षेत्र (वैकल्पिक)" placeholderTextColor={c.textSoft} value={notes} onChangeText={setNotes} multiline numberOfLines={4} style={{ borderWidth: 1, borderColor: c.border, borderRadius: 12, padding: 12, color: c.text, minHeight: 100 }} />
