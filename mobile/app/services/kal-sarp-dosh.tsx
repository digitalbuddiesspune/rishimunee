...
import { generateKundli, kalSarpDosh } from "@services/api/kundli";
import { getServiceBySlug } from "@services/api/services";
import { initiatePayment } from "@services/api/payments";

export default function KalSarpDoshScreen() {
  ...
  useEffect(() => {
    (async () => {
      try { const svc = await getServiceBySlug("kal-sarp-dosh"); const paid = !!svc?.isPaid; setIsPaid(paid); if (paid) { setServicePrice(svc.basePrice || 0); setActivated(false); } else { setActivated(true); } } catch {}
    })();
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await kalSarpDosh({ chart: kundli.chart });
      setResult(res);
        ...
        <ScrollView contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
          <Card>
            {isPaid && servicePrice !== null && (
              <View style={{ marginBottom: 12 }}>
                ...
                <TouchableOpacity onPress={async () => {
                  setPaying(true);
                  try { await initiatePayment({ serviceType: "kal_sarp_dosh", items: [], gateway: "wallet" }); setActivated(true); Alert.alert("Activated", "Kal Sarp Dosh activated via wallet"); }
            ...
            <TouchableOpacity onPress={submit} disabled={loading || (isPaid && !activated)} style={{ backgroundColor: c.primary, padding: 12, borderRadius: 12, opacity: (isPaid && !activated) ? 0.6 : 1 }}>
              <Text style={{ color: c.primaryForeground, textAlign: "center", fontWeight: "700" }}>{(isPaid && !activated) ? "Activate to Continue" : (loading ? "Analyzing..." : "Analyze Kal Sarp Dosh")}</Text>
            </TouchableOpacity>
          </Card>
