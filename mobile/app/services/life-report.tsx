...
import { generateLifeReport } from "@services/api/serviceReports";

export default function LifeReportScreen() {
  ...
  useEffect(() => {
    (async () => { try { const svc = await getServiceBySlug("life-report"); const paid = !!svc?.isPaid; setIsPaid(paid); if (paid) { setServicePrice(svc.basePrice || 0); setActivated(false); } else { setActivated(true); } } catch {} })();
  }, []);
  ...
  const submit = async () => {
    ...
    setResult(null);
    try {
      const data = await generateLifeReport({ notes, ...form });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: c.background }}>
      <Header title="Life Report (जीवन रिपोर्ट)" />
      <KeyboardAvoidingView behavior={Platform.select({ ios: "padding", android: undefined })} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
          <Card>
            {isPaid && servicePrice !== null && !activated && (
              <View style={{ marginBottom: 12 }}>
                ...
                <TouchableOpacity onPress={async () => {
                  setPaying(true);
                  try { await initiatePayment({ serviceType: "life_report", items: [], gateway: "wallet" }); setActivated(true); Alert.alert("Activated", "Life report activated via wallet"); }
            ...
            <Text style={{ fontWeight: "700", marginBottom: 8 }}>Focus Notes</Text>
            <TextInput placeholder="Add any focus areas for the report (optional) • रिपोर्ट के लिए फोकस क्षेत्र (वैकल्पिक)" placeholderTextColor={c.textSoft} value={notes} onChangeText={setNotes} multiline numberOfLines={4} style={{ borderWidth: 1, borderColor: c.border, borderRadius: 12, padding: 12, color: c.text, minHeight: 100 }} />
            ...
            {result && (
              <View style={{ marginTop: 12 }}>
                ...
                <Markdown style={{ body: { color: c.text } }}>{result}</Markdown>
              </View>
            )}
            <Markdown style={{ body: { color: c.text } }}>{`Your report will appear in your **Reports** section. (आपकी रिपोर्ट **Reports** सेक्शन में भी सेव होगी)`}</Markdown>
          </Card>
