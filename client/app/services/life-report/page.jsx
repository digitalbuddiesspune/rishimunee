...
function LifeReportContent() {
  ...
  const { register, handleSubmit, formState: { isSubmitting }, reset } = useForm({
    defaultValues: { name: "", date: "", time: "", place: "", concerns: "" }
  });
  ...
  const onSubmit = async (values) => {
    try {
      setErrorMessage("");
      if (!alreadyPaid) {
        await apiClient.post("/payments/initiate", { serviceType: "life_report", gateway: PAYMENT_GATEWAYS.WALLET });
      }
      const gen = await apiClient.post("/kundli/generate", values);
      const chart = gen?.data?.data?.chart;
      const { data } = await apiClient.post("/kundli/life-report", { chart, concerns: values.concerns });
      setResult({ chart, narrative: data?.data?.narrative });
      dispatch(fetchWallet());
    } catch (error) {
      const message = error.response?.data?.message || "Failed to generate life report";
    ...
    return (
      <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-12 md:grid-cols-12">
        <aside className="md:col-span-4 lg:col-span-3">
          <ServiceNav />
          <ServiceHistory serviceType="life_report" onSelect={(r) => { setFromHistory(true); setResult({ chart: r.result?.chart || null, narrative: r.result?.narrative }); }} />
        </aside>
        <main className="md:col-span-8 lg:col-span-9">
          <Card className="h-[70vh] flex flex-col">
            <CardHeader><CardTitle>Life Report</CardTitle></CardHeader>
  ...
  return (
    <div className="mx-auto grid w-full max-w-5xl gap-8 px-4 py-12 md:grid-cols-[1.1fr,0.9fr]">
      <LoadingOverlay show={isSubmitting} label="Generating life report..." />
      <Card>
        <CardHeader><CardTitle>Life Report</CardTitle></CardHeader>
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <TextArea rows={4} placeholder="Areas to focus (health, wealth, relationships, career, family, spirituality)" {...register("concerns", { required: false })} />
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-[color:var(--color-text-soft)]">
          <p>Comprehensive जीवन रिपोर्ट covering health, धन, परिवार, और करियर with practical remedies and a 10-year outlook.</p>
